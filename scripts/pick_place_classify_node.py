...
class PickPlaceClassifyNode:
    def __init__(self, robot_name):
        ...
        # self.transformation = T.Compose([T.Resize(640), 
        #                                     T.CenterCrop(224), 
        ...
        self.clf = YOLO("./../models/pick_place_yolo_082125.pt")  
    ...
    def pick_place_prediction(self, cv2_img):
        ...
        prediction = self.clf.predict(cv2_img, verbose=False)
        pred_class = prediction[0].names[prediction[0].probs.top1]
        ...
        if pred_class == 'none' or pred_conf <= 0.7: # not in hand if unsure
            return 0 # not in hand
        else: # if pred_class == '1stud' or pred_class == '2stud':
            return 1  # in hand
